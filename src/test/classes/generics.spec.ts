...
describe('generics', () => {
  ...
  describe('typemap', () => {
    const t = TransformerInstance.withDefaultProject();

    // eslint-disable-next-line @typescript-eslint/explicit-function-return-type, @typescript-eslint/no-unused-vars
    function createDto<TestTypeParam>() {
      @t.transformerDecorator()
      class GenericEmbed<U> {
        property1!: U;
      }

      @t.transformerDecorator()
      class Test {
        test!: GenericEmbed<string>;
      }

      return Test;
    }

    it('should filter type parameters from surrounding namespace', () => {
      const dto = createDto();
      const { tree } = t.getPropertyTypeTreesFromConstructor(dto)[0];

      expect(tree).toEqual(
        RootNodeFixture.createRequired({
          children: [
            ClassNodeFixture.create('GenericEmbed', {
              from: 'class',
